...
def run_analysis(data_dir: str = None):
    ...
    logs = pd.read_csv(os.path.join(data_dir, "logs_clean.csv"))

    if logs.empty:
        raise ValueError("logs_clean.csv is empty.")

    logs["timestamp"] = pd.to_datetime(logs["timestamp"], errors="coerce")
    logs = logs.dropna(subset=["timestamp"])

    # FIX: create "hour" column HERE before anything else uses it
    ...
    # FIX: use error_requests["hour"] not logs["hour"]
