...
def run_analysis(data_dir: str = None):
    ...
    ads = pd.read_csv(os.path.join(data_dir, "ads_clean.csv"))

    if ads.empty:
        raise ValueError("ads_clean.csv is empty.")

    ads.columns = ads.columns.str.strip().str.lower()

    if "datetime" in ads.columns:
        ads["datetime"] = pd.to_datetime(
            ads["datetime"],
            dayfirst=True,
            errors="coerce"
        )
    ...
    if "datetime" in ads.columns:
        ...
        hourly_data["ctr"] = (
            hourly_data["clicks"] / hourly_data["impressions"]
        ) * 100
